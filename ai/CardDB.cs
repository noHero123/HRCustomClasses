        ...
        public class Card
        {
            ...
            public bool canplayCard(Playfield p)
            {
                ...
                if (haveToDoRequires)
                {
                    ...
                    {
                        if (p.ownMinions.Count >= 8 - this.needEmptyPlacesForPlaying)
                        {
                            return false;
                        }
                    }

                    if (isRequirementInList(CardDB.ErrorType2.REQ_MINION_CAP_IF_TARGET_AVAILABLE))
                    {
                        if (getTargetsForCard(p).Count >= 1)
                        {

                            if (p.ownMinions.Count >= 8 - this.needMinionsCapIfAvailable)
                            {
                                return false;
                            }
                        }
                    ...
                    if (isRequirementInList(CardDB.ErrorType2.REQ_DAMAGED_TARGET))
                    {
                        ...
                        foreach (Minion m in p.enemyMinions)
                        {
                            if (m.wounded) foundWound = true;
                        }
                        foreach (Minion m in p.ownMinions)
                        {
                            if (m.wounded) foundWound = true;
                        }
                        if (foundWound == false) return false;
                    }
                    if (isRequirementInList(CardDB.ErrorType2.REQ_UNDAMAGED_TARGET))
                    {
                        bool foundUnWound = false;
                        foreach (Minion m in p.enemyMinions)
                        {
                            if (!m.wounded) foundUnWound = true;
                        }
                        foreach (Minion m in p.ownMinions)
                        {
                            if (!m.wounded) foundUnWound = true;
                        }
                        if (foundUnWound == false) return false;
                    }
                    ...
                    if (isRequirementInList(CardDB.ErrorType2.REQ_TARGET_MAX_ATTACK))
                    {
                        ...
                        if (maxattack == false) return false;
                    }
                    ...
                    if (isRequirementInList(CardDB.ErrorType2.REQ_TARGET_WITH_RACE))
                    {
                        ...
                        if (foundRace == false) return false;
                    }
                    */
