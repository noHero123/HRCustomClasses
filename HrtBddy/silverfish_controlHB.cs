...
namespace Silverfish
{
    public class Bot : ICustomDeck
    {
        ...
        //change the following 2 functions, to change botbehavior (rush/control)
        ...
        public int getPlayfieldValue(Playfield p)
        {
            ...
            {
                retval -= (hpboarder + 1 - p.ownHeroHp - p.ownHeroDefence)*
                          (hpboarder + 1 - p.ownHeroHp - p.ownHeroDefence);
            }
            ...
            foreach (Minion m in p.ownMinions)
            {
                ...
                if (m.handcard.card.name == CardDB.cardName.direwolfalpha ||
                    ...
                    m.handcard.card.name == CardDB.cardName.raidleader) retval += 10;
            ...
            /*if (p.enemyMinions.Count >= 0)
          {
            ...
            foreach (Action a in p.playactions)
            {
                ...
                if (a.useability && a.handcard.card.name == CardDB.cardName.lesserheal &&
                    ((a.enemytarget >= 10 && a.enemytarget <= 20) || a.enemytarget == 200)) retval -= 5;
                if (!a.cardplay) continue;
                if ((a.handcard.card.name == CardDB.cardName.thecoin ||
                     a.handcard.card.name == CardDB.cardName.innervate)) usecoin = true;
                ...
                if (p.ownHeroName != HeroEnum.thief && a.handcard.card.type == CardDB.cardtype.SPELL &&
                    (a.numEnemysBeforePlayed == 0 || a.enemytarget == 200) &&
                    a.handcard.card.name != CardDB.cardName.shieldblock) retval -= 11;
                if (p.ownHeroName == HeroEnum.thief && a.handcard.card.type == CardDB.cardtype.SPELL &&
                    (a.enemytarget == 200)) retval -= 11;
            }
            ...
            {
                retval -= getEnemyMinionValue(m, p);
            }
            ...
            {
                if (!a.cardplay) continue;
                if (a.handcard.card.name == CardDB.cardName.soulfire ||
                    a.handcard.card.name == CardDB.cardName.doomguard ||
                    a.handcard.card.name == CardDB.cardName.succubus) deletecardsAtLast = 1;
                if (deletecardsAtLast == 1 &&
                    !(a.handcard.card.name == CardDB.cardName.soulfire ||
                      a.handcard.card.name == CardDB.cardName.doomguard ||
                      a.handcard.card.name == CardDB.cardName.succubus)) retval -= 20;
            }
